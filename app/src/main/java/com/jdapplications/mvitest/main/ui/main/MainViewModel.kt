...
class MainViewModel(private val mainActionProcessorHolder: MainActionProcessorHolder) : ViewModel(), MviViewModel<MainIntent, MainViewState> {
    ...
    private fun actionFromIntent(intent: MainIntent): MainAction {
        return when (intent) {
            ...
            is MainIntent.UpdateStatusIntent -> MainAction.UpdateStatusAction
        }
    }
    ...
    companion object {
        ...
        private val reducer = BiFunction { previousState: MainViewState, result: MainResult ->
            when (result) {
                is MainResult.UpdateStatusResults.Success -> previousState
                        .copy(userList = listOf("Status"), loading = false)
                is MainResult.UpdateStatusResults.Failure -> previousState
                        .copy(error = true, loading = false)
                is MainResult.UpdateStatusResults.Loading -> previousState
                        .copy(loading = true)
                MainResult.InitialResult -> previousState
            }
        }
    }
